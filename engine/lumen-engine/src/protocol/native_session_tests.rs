...
fn generation_three_golden_hello_and_plan_bytes_are_stable() {
    ...
    assert_eq!(
        hello_bytes,
        vec![
            194, 1, 8, 7, 82, 189, 1, 8, 3, 16, 3, 32, 128, 30, 40, 240, 16, 48, 192, 169, 7, 58,
    ...
    assert_eq!(
        plan_bytes,
        vec![
            164, 1, 8, 7, 82, 159, 1, 8, 3, 16, 132, 134, 136, 8, 32, 128, 30, 40, 240, 16, 48,
...
#[test]
fn generation_three_rejects_generation_two_hello_at_version_negotiation() {
    // Given: a byte-for-byte generation-two hello from the retired schema.
    ...
    // Then: protocol generation fails before a session plan can be created.
...
fn negotiates_the_exact_account_selected_hevc_hdr_profile() {
    ...
    assert_eq!(plan.protocol_version, 3);
