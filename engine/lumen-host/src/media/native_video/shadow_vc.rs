...
const LUMA_MODEL: &str = "fc3-luma16-v1";
const LUMA_CHECKPOINT: &str = "af2da39131bfd7df42f0bc5942025e56effae15d7b68ef1ad79e9d428a92e052";
...
pub(super) fn validate(format: PlatformVideoFormat, bytes: &[u8]) -> Result<(), String> {
    ...
    if bytes.len() > 1024 || format.codec != PlatformVideoCodec::ShadowVc
        ...
        || format.chroma_subsampling != PlatformChromaSubsampling::Yuv420
        || format.bit_depth != 10
        || (format.dynamic_range != PlatformDynamicRange::Sdr && format.profile != PlatformVideoProfile::ShadowVcLuma16)
        || format.color_range != PlatformColorRange::Limited {
...
pub(super) fn configuration_from_frame(bytes: &[u8]) -> Result<Vec<u8>, String> {
    if bytes.starts_with(b"SCV3") { return luma_configuration_from_frame(bytes); }
...
fn luma_configuration_from_frame(bytes: &[u8]) -> Result<Vec<u8>, String> {
    ...
    if id == 0 || reference >= id || !dimensions(width.into(), height.into()) {
        return Err("invalid SCV3 geometry or reference".into());
    }
...
mod tests {
    ...
    fn luma16_envelope_binds_range_model_and_reference() {
        for (range, dynamic_range) in [(1_u32, PlatformDynamicRange::Sdr), (2, PlatformDynamicRange::Hdr10)] {
            ...
            for index in 0..bytes.len() { let mut bad = bytes.clone(); bad[index] ^= 1; assert!(configuration_from_frame(&bad).is_err()); }
        }
    }
    #[test]
