...
impl ControlRouter {
    ...
    pub(crate) fn maybe_request_native_video_periodic(
        ...
        // Integer-reference profiles have no accumulated reconstruction drift.
        // Gaps/CRC failures already request acknowledged independent
        ...
        if pending.plan.selected_video_capability.as_ref()
            ...
            .is_some_and(|format| matches!(
                ...
                Ok(NativeVideoProfile::ShadowVcRegionalPredictor8 | NativeVideoProfile::ShadowVcPixel10History)
            ))
        {
            return Ok(false);
        }
...
fn platform_video_format(plan: &HostSessionPlan) -> Option<PlatformVideoFormat> {
    ...
    Some(PlatformVideoFormat {
        ...
        profile: match NativeVideoProfile::try_from(selected.profile).ok()? {
            ...
            NativeVideoProfile::ShadowVcPixel10History => PlatformVideoProfile::ShadowVcPixel10,
            NativeVideoProfile::Unspecified | NativeVideoProfile::ShadowVcPixel10 | NativeVideoProfile::ShadowVcPixel10Motion128 | NativeVideoProfile::ShadowVcPixel10Dc => return None,
        },
...
mod periodic_idr_tests {
    ...
    fn integer_reference_profiles_skip_periodic_refresh_but_allow_repair() {
        ...
        for profile in [NativeVideoProfile::ShadowVcRegionalPredictor8, NativeVideoProfile::ShadowVcPixel10History] {
            ...
            pending.plan.selected_video_capability.as_mut().unwrap().format.as_mut().unwrap().profile = profile as i32;
            pending.acknowledged_generation_id = Some(1);
            pending.periodic_idr.schedule_after_generation_ack(Instant::now() - Duration::from_secs(5));
            assert!(!router.maybe_request_native_video_periodic(context.session_epoch, false).unwrap());
            assert!(!router.native.pending.as_ref().unwrap().periodic_idr.is_outstanding());
