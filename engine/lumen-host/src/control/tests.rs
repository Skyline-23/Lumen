...
#[test]
fn generation_two_hello_is_rejected_before_pending_session_mutation() {
    // Given: an authenticated generation-two-shaped hello for an available application.
    let platform = Arc::new(RecordingPlatformSessionControl::default());
    let (_root, mut router) = router_with_platform(platform.clone());
    router
        .authorities()
    ...
    // When: the stale hello reaches the control router.
    let responses = router.dispatch_native_control(
        ClientControlEnvelope {
            request_id: 77,
    ...
    // Then: version negotiation fails and no pending or platform session is created.
    ...
    println!(
        "generation-two envelope rejection: {:?}",
        NativeNegotiationFailure::try_from(error.negotiation_failure).unwrap()
    );
    assert_eq!(
