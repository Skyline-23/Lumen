...
struct ProbeMetrics {
  ...
  uint64_t tileBoundedPresentationDepth4Updates = 0;
  uint64_t tileBoundedPresentationDepth8Updates = 0;
  uint64_t tileBoundedPresentationDepth4DroppedStale = 0;
  uint64_t tileBoundedPresentationDepth8DroppedStale = 0;
  double tileBoundedPresentationDepth4MaxSkewMilliseconds = 0.0;
  double tileBoundedPresentationDepth8MaxSkewMilliseconds = 0.0;
...
struct TileBoundedPresentationPolicy {
  double budgetMilliseconds = 16.667;
  uint64_t maxQueueDepth = 4;
...
struct TileBoundedPresentationProgress {
  TileBoundedPresentationPolicy depth4 {16.667, 4};
  TileBoundedPresentationPolicy depth8 {16.667, 8};
};
...
bool bootstrapBoundedPresentation(TileBoundedPresentationPolicy &policy) {
  ...
  for (uint32_t tileIndex : policy.observedTileIndexes) {
    ...
    candidate[tileIndex] = queueEntry->second.back();
  }
  ...
  for (const auto &entry : candidate) {
    ...
    policy.pendingDisplayTimesByTile[entry.first].clear();
  }
...
bool presentBestBoundedTile(TileBoundedPresentationPolicy &policy, uint32_t tileIndex) {
  ...
  size_t bestIndex = 0;
  uint64_t bestDisplayTime = 0;
  double bestSkewMilliseconds = 0.0;
  for (size_t index = 0; index < queueEntry->second.size(); ++index) {
    const uint64_t displayTime = queueEntry->second[index];
    ...
    if (displayedEntry != policy.displayedDisplayTimeByTile.end() &&
        displayTime <= displayedEntry->second) {
      continue;
    }
    ...
    if (!foundCandidate || displayTime > bestDisplayTime) {
      foundCandidate = true;
      bestIndex = index;
      bestDisplayTime = displayTime;
      bestSkewMilliseconds = candidateSkewMilliseconds;
    }
  }
  ...
  policy.displayedDisplayTimeByTile[tileIndex] = bestDisplayTime;
  policy.presentedUpdates += 1;
  policy.maxPresentedSkewMilliseconds =
    std::max(policy.maxPresentedSkewMilliseconds, bestSkewMilliseconds);
  policy.droppedStaleUpdates += static_cast<uint64_t>(bestIndex);
  queueEntry->second.erase(queueEntry->second.begin(), queueEntry->second.begin() + bestIndex + 1);
  return true;
}

void updateBoundedPresentationPolicy(
  TileBoundedPresentationPolicy &policy,
  ...
  while (queue.size() > static_cast<size_t>(policy.maxQueueDepth)) {
    queue.pop_front();
    policy.droppedStaleUpdates += 1;
  }

  bootstrapBoundedPresentation(policy);
  ...
  while (presentedAny) {
    ...
    for (uint32_t candidateTileIndex : tileIndexes) {
      presentedAny = presentBestBoundedTile(policy, candidateTileIndex) || presentedAny;
    }
  }
}

void updateBoundedPresentation(
  TileBoundedPresentationProgress &presentation,
  const LumenCoreEncodedCaptureFrameRecord &record
) {
  updateBoundedPresentationPolicy(presentation.depth4, record);
  updateBoundedPresentationPolicy(presentation.depth8, record);
}
...
void drainForwardedFrames(
  ...
  TileBoundedPresentationProgress &boundedPresentation,
  ...
  while (true) {
    ...
    updateBoundedPresentation(boundedPresentation, record);
...
int main(int argc, const char *argv[]) {
  @autoreleasepool {
    ...
    metrics.tileBoundedPresentationDepth4Updates =
      boundedPresentation.depth4.presentedUpdates;
    metrics.tileBoundedPresentationDepth8Updates =
      boundedPresentation.depth8.presentedUpdates;
    metrics.tileBoundedPresentationDepth4DroppedStale =
      boundedPresentation.depth4.droppedStaleUpdates;
    metrics.tileBoundedPresentationDepth8DroppedStale =
      boundedPresentation.depth8.droppedStaleUpdates;
    metrics.tileBoundedPresentationDepth4MaxSkewMilliseconds =
      boundedPresentation.depth4.maxPresentedSkewMilliseconds;
    metrics.tileBoundedPresentationDepth8MaxSkewMilliseconds =
      boundedPresentation.depth8.maxPresentedSkewMilliseconds;
    ...
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_BOUNDED_PRESENTATION_DEPTH4_UPDATES=%llu\n",
      metrics.tileBoundedPresentationDepth4Updates
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_BOUNDED_PRESENTATION_DEPTH8_UPDATES=%llu\n",
      metrics.tileBoundedPresentationDepth8Updates
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_BOUNDED_PRESENTATION_DEPTH4_DROPPED_STALE=%llu\n",
      metrics.tileBoundedPresentationDepth4DroppedStale
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_BOUNDED_PRESENTATION_DEPTH8_DROPPED_STALE=%llu\n",
      metrics.tileBoundedPresentationDepth8DroppedStale
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_BOUNDED_PRESENTATION_DEPTH4_MAX_SKEW_MS=%.3f\n",
      metrics.tileBoundedPresentationDepth4MaxSkewMilliseconds
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_BOUNDED_PRESENTATION_DEPTH8_MAX_SKEW_MS=%.3f\n",
      metrics.tileBoundedPresentationDepth8MaxSkewMilliseconds
    );
