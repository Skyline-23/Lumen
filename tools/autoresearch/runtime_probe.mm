...
struct ProbeMetrics {
  ...
  uint64_t tileNearestPhaseOneFramePairs = 0;
  uint64_t tileNearestPhaseTwoFramePairs = 0;
  uint64_t tileNearestPhaseThreeFramePairs = 0;
  double tileNearestPhaseTwoFrameMaxSkewMilliseconds = 0.0;
...
uint64_t countNearestPhasePairs(
  std::vector<uint64_t> tile0DisplayTimes,
  std::vector<uint64_t> tile1DisplayTimes,
  double budgetMilliseconds,
  double *maxSkewMilliseconds
) {
  std::sort(tile0DisplayTimes.begin(), tile0DisplayTimes.end());
  std::sort(tile1DisplayTimes.begin(), tile1DisplayTimes.end());

  uint64_t pairs = 0;
  size_t tile0Index = 0;
  size_t tile1Index = 0;
  while (tile0Index < tile0DisplayTimes.size() && tile1Index < tile1DisplayTimes.size()) {
    const uint64_t tile0Time = tile0DisplayTimes[tile0Index];
    const uint64_t tile1Time = tile1DisplayTimes[tile1Index];
    const uint64_t delta = tile0Time >= tile1Time ? tile0Time - tile1Time : tile1Time - tile0Time;
    const double skewMilliseconds = displayTimeDeltaMilliseconds(delta);
...
void computeNearestPhaseMetrics(
  ProbeMetrics &metrics,
  const std::vector<TileArrivalTraceRecord> &trace
) {
  std::vector<uint64_t> tile0DisplayTimes;
  std::vector<uint64_t> tile1DisplayTimes;
  tile0DisplayTimes.reserve(trace.size() / 2);
  tile1DisplayTimes.reserve(trace.size() / 2);
  for (const TileArrivalTraceRecord &record : trace) {
    if (record.tileCount != 2) {
      continue;
    }
    if (record.tileIndex == 0) {
      tile0DisplayTimes.push_back(record.displayTime);
    } else if (record.tileIndex == 1) {
      tile1DisplayTimes.push_back(record.displayTime);
    }
  }

  metrics.tileNearestPhaseOneFramePairs =
    countNearestPhasePairs(tile0DisplayTimes, tile1DisplayTimes, 8.333, nullptr);
  metrics.tileNearestPhaseTwoFramePairs =
    countNearestPhasePairs(
      tile0DisplayTimes,
      tile1DisplayTimes,
      16.667,
      &metrics.tileNearestPhaseTwoFrameMaxSkewMilliseconds
    );
  metrics.tileNearestPhaseThreeFramePairs =
    countNearestPhasePairs(tile0DisplayTimes, tile1DisplayTimes, 25.0, nullptr);
}
...
int main(int argc, const char *argv[]) {
  @autoreleasepool {
    ...
    computeNearestPhaseMetrics(metrics, tileArrivalTrace);
    ...
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_NEAREST_PHASE_ONE_FRAME_PAIRS=%llu\n",
      metrics.tileNearestPhaseOneFramePairs
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_NEAREST_PHASE_TWO_FRAME_PAIRS=%llu\n",
      metrics.tileNearestPhaseTwoFramePairs
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_NEAREST_PHASE_THREE_FRAME_PAIRS=%llu\n",
      metrics.tileNearestPhaseThreeFramePairs
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_NEAREST_PHASE_TWO_FRAME_UPDATES=%llu\n",
      metrics.tileNearestPhaseTwoFramePairs * 2
    );
    std::printf(
      "AUTORESEARCH_RUNTIME_PROBE_TILE_NEAREST_PHASE_TWO_FRAME_MAX_SKEW_MS=%.3f\n",
      metrics.tileNearestPhaseTwoFrameMaxSkewMilliseconds
    );
