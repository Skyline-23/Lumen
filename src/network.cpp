...
namespace net {
  ...
  std::uint16_t map_port(int port) {
    ...
    auto mapped_port = (std::uint16_t) ((int) config::runtime.port + port);
   ...
   * @return Hostname-based instance name or "Lumen" if hostname is invalid.
   */
  std::string mdns_instance_name(const std::string_view &hostname) {
    ...
    return !instancename.empty() ? instancename : "Lumen";
  }
}  // namespace net
