...
#undef _CONVERT_2_ARG_
  ...
  void apply_config(std::unordered_map<std::string, std::string> &&vars) {
    ...
    path_f(vars, "file_state", shadow_http.file_state);

    // Must be run after "file_state"
    config::runtime.credentials_file = config::shadow_http.file_state;
    path_f(vars, "credentials_file", config::runtime.credentials_file);

    string_f(vars, "external_ip", shadow_http.external_ip);
    ...
    int_between_f(vars, "port"s, port, {1024 + shadow_http::PORT_HTTPS, 65535 - rtsp_stream::RTSP_SETUP_PORT});
  ...
  int parse(int argc, char *argv[]) {
    ...
    if (!config_loaded && !shortcut_launch) {
      BOOST_LOG(fatal) << "To relaunch Lumen successfully, use the shortcut in the Start Menu. Do not run Lumen.exe manually."sv;
