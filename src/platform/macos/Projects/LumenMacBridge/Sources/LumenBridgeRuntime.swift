...
public struct LumenMacDisplayKitCaptureConfiguration: Equatable, Sendable {
    ...
    var mdkValue: MDKEncodedCaptureConfiguration {
        ...
        return MDKEncodedCaptureConfiguration(
            ...
            tileLayout: effectiveEncodedTileLayout
        )
    }

    public var effectiveEncodedTileLayout: MDKEncodedCaptureTileLayout {
        guard codec == .hevc,
              usesHDRTransport || negotiatedDynamicRangeTransport == LumenCoreDynamicRangeTransportSDRBaseHDROverlay,
              effectivePreprocessStrategy == .none else {
            return .singleFrame
        }

        return MDKEncodedCaptureTileLayout(tileCount: 1, encodedLaneCount: 2)
    }
