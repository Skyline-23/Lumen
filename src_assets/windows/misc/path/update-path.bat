...
if "%~1"=="" (
    ...
    echo   add    - Adds Lumen directories to system PATH
    echo   remove - Removes Lumen directories from system PATH
    exit /b 1
)

rem Get Lumen root directory
for %%I in ("%~dp0\..") do set "ROOT_DIR=%%~fI"
echo Lumen root directory: !ROOT_DIR!
...
if /i "%~1"=="add" (
    ...
    if "!NEW_PATH!" neq "!CURRENT_PATH!" (
        ...
        if !ERRORLEVEL!==0 (
            echo Successfully added Lumen directories to PATH
        ) else (
            echo Failed to add Lumen directories to PATH
        )
    ) else (
...
if /i "%~1"=="remove" (
    ...
    if "!CHANGES_MADE!"=="1" (
        ...
        if !ERRORLEVEL!==0 (
            echo Successfully removed Lumen directories from PATH
        ) else (
            echo Failed to remove Lumen directories from PATH
        )
    ) else (
        echo No changes needed to PATH
    )
