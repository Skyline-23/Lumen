...
- Ordered-output branch: receiver-side temporal arbitration is now preferred over sender-side paired emission, but the naive latest-pending budget and heavy probe-side queues are closed. The next useful version should collect a lightweight arrival trace or implement receiver-side phase pairing outside the capture hot path, with bounded queue depth and a two-frame target.
