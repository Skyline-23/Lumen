...
- Ordered-output branch: receiver-side temporal arbitration is now preferred over sender-side paired emission, but the naive latest-pending budget, heavy probe-side queues, FIFO-front offline pairing, and source-time nearest matching are closed. The next useful version should measure tile arrival/callback-drain phase or nearest-in-window pairing against a receiver presentation clock, with bounded queue depth and a two-frame target.
