...
- Ordered-output branch: receiver-side temporal arbitration is now preferred over sender-side paired emission, but the naive latest-pending budget, heavy probe-side queues, and FIFO-front offline pairing are closed. The next useful version should measure tile phase distribution or nearest-in-window pairing against a reconstructed receiver clock, with bounded queue depth and a two-frame target.
