...
- Ordered-output branch: receiver-side temporal arbitration is now preferred over sender-side paired emission, but the naive latest-pending budget, heavy probe-side queues, FIFO-front offline pairing, source-time nearest matching, and arrival-time nearest matching are closed. The next useful version should model stateful partial presentation updates with bounded queue depth, not search for more whole-tile pairs in timestamp sequences.
