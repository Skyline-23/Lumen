...
- Ordered-output branch: investigate paired tile emission or receiver-side temporal arbitration so enqueue order does not present one lane as persistently stale. Reverse ordering is diagnostic-only and should not be kept as a fix.
