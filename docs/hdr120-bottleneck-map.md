...
7. The skew distribution is lane-asymmetric: tile 1 is stale in most composite updates. That makes the next structural question narrower: determine why lane 1 trails even though prior lane counters looked symmetric, or make the receiver contract robust to one consistently older tile.
...
- Lane-staleness branch: investigate why tile 1 is almost always the older tile in the freshness composite despite prior symmetric lane callback counts. Useful probes should stay discard-only and should compare lane region cost, lane output ordering, and callback drain order without changing bitrate, HDR, resolution, or target frame rate.
